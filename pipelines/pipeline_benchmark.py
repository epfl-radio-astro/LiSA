...
# python pipeline_benchmark.py SDC2_benchmark.config 144 1
...
@profile
def read_data(n_domains, domain_index, input_datacube, domain_border,  square_domains):
    ...
    return reader_original, reader_denoised

print(">>>   READING DATA")
reader_original, reader_denoised = read_data(n_domains, domain_index, input_datacube, domain_border,  square_domains)
...
@profile
def denoise_data(denoised_file_name, reader_denoised):
    ...
    return reader_denoised

print(">>>   DENOISING")
reader_denoised = denoise_data(denoised_file_name, reader_denoised)
######################################################
# Step 3: Find sources
######################################################

@profile
def find_sources(source_candidates_file_name, reader_denoised):
...
find_sources(source_candidates_file_name, reader_denoised)
...
@profile
def merge_sources(source_candidates_file_name, merged_source_candidates_file_name):
    merge_source_candidates(source_candidates_file_name, merged_source_candidates_file_name)
print(">>>   MERGE SOURCES")
merge_sources(source_candidates_file_name, merged_source_candidates_file_name)
...
@profile
def classify_and_regress(merged_source_candidates_file_name, reader_original):
...
print(">>>   AI")
classify_and_regress(merged_source_candidates_file_name, reader_original) 

final_time = time.time() 
print("Profiling: final time {0}".format(final_time - start_time))
